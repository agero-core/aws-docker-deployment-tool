...
prod_table = resource.Table('ECS_Inventory_Production_West')
...
elbclient_prod = boto3.client('elbv2', aws_access_key_id=aws_access_key_id,aws_secret_access_key=aws_secret_access_key,aws_session_token=aws_session_token,region_name='us-west-2')
sns_prod = boto3.client('sns', aws_access_key_id=aws_access_key_id,aws_secret_access_key=aws_secret_access_key,aws_session_token=aws_session_token,region_name='us-west-2')
### ###
...
def sendnotification(appname, prod1, prod2, prod_acc, version):
    ...
    print sns_arn
    sns_message = "Updated West Green Environment: " + env_name + "\n" + "Running Version: " + version 
    subject = "Swap on Application " + appname + " has been perfromed in West Region"
...
def recordswap(appname):
    ...
    dynamo_response = dynamo_client.get_item(TableName="ECS_Inventory_Production_West", Key={'ApplicationName': {'S': appname}, 'Environment': {'S': 'PROD1'}}, AttributesToGet=['TechnicalTeam', 'Time', 'URL', 'Version'])
    ...
    dynamo_response = dynamo_client.get_item(TableName="ECS_Inventory_Production_West", Key={'ApplicationName': {'S': appname}, 'Environment': {'S': 'PROD2'}}, AttributesToGet=['TechnicalTeam', 'Time', 'URL', 'Version'])
    url = dynamo_response['Item']['URL']['S']
    techteam = dynamo_response['Item']['TechnicalTeam']['S']
    if 'prod-blue' in url:
        time_blue = dynamo_response['Item']['Time']['S']
    ...
    if prod1 == 'green':
        resp = dynamo_client.update_item(TableName="ECS_Inventory_Production_West", Key={'ApplicationName': {'S': appname}, 'Environment': {'S': 'PROD1'}}, AttributeUpdates={'URL':{'Value':{'S': url_blue}}, 'Time':{'Value':{'S': str(time)}}})
        print resp

        resp = dynamo_client.update_item(TableName="ECS_Inventory_Production_West", Key={'ApplicationName': {'S': appname}, 'Environment': {'S': 'PROD2'}}, AttributeUpdates={'URL':{'Value':{'S': url_green}}, 'Time':{'Value':{'S': str(time)}}})
        print resp

    else:
        resp = dynamo_client.update_item(TableName="ECS_Inventory_Production_West", Key={'ApplicationName': {'S': appname}, 'Environment': {'S': 'PROD2'}}, AttributeUpdates={'URL':{'Value':{'S': url_blue}}, 'Time':{'Value':{'S': str(time)}}})
        print resp

        resp = dynamo_client.update_item(TableName="ECS_Inventory_Production_West", Key={'ApplicationName': {'S': appname}, 'Environment': {'S': 'PROD1'}}, AttributeUpdates={'URL':{'Value':{'S': url_green}}, 'Time':{'Value':{'S': str(time)}}})
        print resp        
...
def lambda_handler(event, context):
    ...
    message = "Starting Prod Blue/Green Swap for " + appname + " in West Region"
    #time = datetime.datetime.now()
    ...
    message = "Prod Blue/Green Swap in West Region Completed Successfully for " + appname
        #time = datetime.datetime.now()
        #resp = dynamo_client.put_item(TableName="DevOpsLogsTable", Item={'ResourceName': {'S': 'swapprod'}, 'Time': {'S': str(time)}, 'Message': {'S': message}})
        #print resp
    status_code = 200
