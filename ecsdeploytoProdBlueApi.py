...
def lambda_handler(event, context):
    ...
    time = datetime.datetime.now()
    resp = dynamo_client.update_item(TableName="ECS_Inventory_Production", Key={'ApplicationName': {'S': appname}, 'Environment': {'S': env.upper()}}, AttributeUpdates={'Version':{'Value':{'S': version}}, 'Time':{'Value':{'S': str(time)}}})
    print resp
